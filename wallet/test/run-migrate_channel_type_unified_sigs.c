...
static void add_channel(struct db *db, u64 id, const struct channel_type *type)
{
	struct db_stmt *stmt;

	stmt = db_prepare_v2(db, SQL("INSERT INTO channels (id, channel_type)"
				     " VALUES (?, ?);"));
	db_bind_u64(stmt, id);
	if (type)
		db_bind_channel_type(stmt, type);
	else
		db_bind_null(stmt);
	db_exec_prepared_v2(take(stmt));
}

static struct channel_type *get_channel_type(const tal_t *ctx,
					     struct db *db, u64 id)
{
	struct db_stmt *stmt;
	struct channel_type *type;

	stmt = db_prepare_v2(db, SQL("SELECT channel_type FROM channels"
				     " WHERE id = ?;"));
	db_bind_u64(stmt, id);
	...
	if (db_col_is_null(stmt, "channel_type")) {
		...
		type = db_col_channel_type(ctx, stmt, "channel_type");
	tal_free(stmt);
	return type;
}
...
int main(int argc, const char *argv[])
{
	...
	struct channel_type *old, *unrelated, *got;
	...
	/* A channel opened while option_unified_sigs was bit 70. */
	old = channel_type_static_remotekey(tmpctx);
	set_feature_bit(&old->features,
			COMPULSORY_FEATURE(OLD_OPT_UNIFIED_SIGS));
	add_channel(w->db, 1, old);

	/* One which never had it, and one with no type at all. */
	unrelated = channel_type_static_remotekey(tmpctx);
	add_channel(w->db, 2, unrelated);
	add_channel(w->db, 3, NULL);

	w->db->data_version = 0;
	db_set_intvar(w->db, "data_version", w->db->data_version);
	migrate_channel_type_unified_sigs_bit(ld, w->db);

	/* The old bit is gone and the new one is set, in its even form. */
	got = get_channel_type(tmpctx, w->db, 1);
	assert(got);
	assert(!feature_offered(got->features, OLD_OPT_UNIFIED_SIGS));
	assert(feature_is_set(got->features,
			      COMPULSORY_FEATURE(OPT_UNIFIED_SIGS)));
	/* And nothing else in that byte was disturbed. */
	assert(feature_is_set(got->features,
			      COMPULSORY_FEATURE(OPT_STATIC_REMOTEKEY)));

	/* Untouched. */
	got = get_channel_type(tmpctx, w->db, 2);
	assert(got);
	assert(!feature_offered(got->features, OPT_UNIFIED_SIGS));
	assert(feature_is_set(got->features,
			      COMPULSORY_FEATURE(OPT_STATIC_REMOTEKEY)));
	assert(get_channel_type(tmpctx, w->db, 3) == NULL);
